...
psi_new = arrayfun(@(I) X_new{Pairs(I,1)} .^ 2 / 2 - X_new{Pairs(I,2)} .^ 2 / 2,1:nchoosek(m,2),'UniformOutput',false);
psi_new = sum(cat(1,psi_new{:}),1); % energy function
V_new = cell(m,1);
[V_new{:}] = deal(zeros(1,n));
for i = 1:m % define ith velocity component
    for j = 1:m
        if j ~= i
            V_new{i} = V_new{i} - X_new{j} * E_new{i}; % velocity along ith basis vector due to jth distance
        end
    end
end
...
psi_new_rev = arrayfun(@(I) X_new{Pairs(I,1)} .^ 2 / 2 - X_new{Pairs(I,2)} .^ 2 / 2,1:nchoosek(m,2),'UniformOutput',false);
psi_new_rev = sum(cat(1,psi_new_rev{:}),1); % energy function
V_new = cell(m,1);
[V_new{:}] = deal(zeros(1,n));
for i = 1:m % define ith velocity component
    for j = 1:m
...
psi_rev = zeros(1,npoints);
psi_rev(1) = psi_new_rev;
for k = 2:npoints
...
if n == 2
    ...
    surf(x{1},x{2},Psi,'EdgeColor','none','FaceColor','interp','FaceAlpha',0.3);
    view(15,45)
    hold on
    % plot3(gamma(1,:),gamma(2,:),c ^ 2 / 2 * ones(size(gamma,2)),'LineWidth',2,'Color','k')
    plot3(gamma(1,:),gamma(2,:),psi,'LineWidth',2,'Color','k')
    quiver3(x{1},x{2},Psi,v{1},v{2},zeros(size(Psi)),'Color','k','LineWidth',1);
    axis([0,c,0,c,-Inf,Inf])
    % daspect([1,1,1])
    
    ax = gca;
    ax.XLabel.String = '$$x$$';
    ax.YLabel.String = '$$y$$';
    ax.ZLabel.String = '$$\Psi$$';
    ...
    if save
        exportgraphics(gcf,sprintf('%s/Potential flow  n = %d, Psi surface, start = (%0.6f,%0.6f).png',pwd,n,c0(1),c0(2)),'Resolution',300)
    end
    ...
    if save
        exportgraphics(gcf,sprintf('%s/Potential flow n = %d, Psi overlaid, start = (%0.6f,%0.6f).png',pwd,n,c0(1),c0(2)),'Resolution',300)
    end
    ...
    quiver3(x{1},x{2},Phi,v{1},v{2},-2 * H,'Color','k','LineWidth',1);
    axis([0,c,0,c,0,Inf])
    % daspect([1,1,1])
    ...
    if save
        exportgraphics(gcf,sprintf('%s/Potential flow n = %d, Phi surface, start = (%0.6f,%0.6f).png',pwd,n,c0(1),c0(2)),'Resolution',300)
    end
    ...
    if save
        exportgraphics(gcf,sprintf('%s/Potential flow n = %d, Phi overlaid, start = (%0.6f,%0.6f).png',pwd,n,c0(1),c0(2)),'Resolution',300)
    end
