...
% PotentialField4(2,'steps',10,'npoints',200,'step_size',5e-3,'save',true)

function PotentialField4(varargin)
...
for c = 0:dc:c_max
    ...
    States = (eye(n) * (c_max + c) + (ones(n) - eye(n)) * (c_max - c)) / sqrt(2) / 2; % coordinates of reference states, scaled to maximum value of c
    % States = eye(n) * c / sqrt(2);
    ...
    c0 = repmat(c_max / sqrt(n) / 2,[1,n]) + repmat(c_max / sqrt(n) / 2,[1,n]) * (r - 1) / steps;
...
if n == 2
    ...
    for i = 1:(steps+1)
        ...
        c = dc * (i - 1);
        States = (eye(n) * (c_max + c) + (ones(n) - eye(n)) * (c_max - c)) / sqrt(2) / 2;
        States = cat(1,States,ones(n) * (c_max + c) / 2 / sqrt(2));
        plot3(gamma(1,:),gamma(2,:),-((c ^ 2 / 2 + (c_max ^ 2 / 4 - c ^ 2 / 4))) * ones(size(gamma,2)),...
            'LineWidth',2,'LineStyle','-','Color',[1,0,1] * (steps + 1 - i + 1) / (steps + 1))
        % quiver3(x{1},x{2},-H,v{1},v{2},zeros(size(H)),'Color','k','LineWidth',1);
        scatter3(States(1,1),States(1,2),-((c ^ 2 / 2 + (c_max ^ 2 / 4 - c ^ 2 / 4))) * ones(size(States,1),1),200,...
            'filled','b','o','MarkerEdgeColor','none','MarkerFaceAlpha',(i) / (steps + 1))
        scatter3(States(2,1),States(2,2),-((c ^ 2 / 2 + (c_max ^ 2 / 4 - c ^ 2 / 4))) * ones(size(States,1),1),200,...
            'filled','r','o','MarkerEdgeColor','none','MarkerFaceAlpha',(i) / (steps + 1))
        scatter3(States(3,1),States(3,2),-((c ^ 2 / 2 + (c_max ^ 2 / 4 - c ^ 2 / 4))) * ones(size(States,1),1),200,...
            'filled','m','o','MarkerEdgeColor','none','MarkerFaceAlpha',(i) / (steps + 1))
        
    end
    axis([0,c,0,c,-c_max ^ 2 / 2,-c_max ^ 2 / 4])
        % daspect([1,1,1])
    ax = gca;
    ax.XLabel.String = '$$x$$';
    ax.YLabel.String = '$$y$$';
